...
| `npm run sensor` | 실시간 센서 모드. 가상 센서로 네 앱을 돌려 보고, 가짜 블루투스에 업체별 패킷을 흘려 어댑터 4종(PASCO·사이언스큐브·EZMaker·Vernier)의 판별·해석을 확인한다. 미지원 브라우저, CSV, 외부 전송 0건 포함 (58항목) |
| `npm run shelf` | 책장·제출·선생님 화면을 데모 모드로 끝까지 돌려 봄. 지우기 확인창·되돌리기 포함 (134항목) |
